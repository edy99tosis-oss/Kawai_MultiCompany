...
Begin VB.Form frmPOOtherScheduled 
   BackColor       =   &H00FDDFE3&
   Caption         =   "PO Other Scheduled"
   ClientHeight    =   11010
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   15240
   Icon            =   "frmPOOtherScheduled.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   11010
   ScaleWidth      =   15240
   ...
   Begin VSFlex8Ctl.VSFlexGrid grid 
      ...
      FormatString    =   $"frmPOOtherScheduled.frx":0E42
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmPOOtherScheduled"
...
'                  "where (tm.trade_cls='2'   or tm.trade_cls='3') " & _
...
'
    ...
    With cboCust
        .clear
        .columnCount = 8
        .ColumnWidths = "60pt;170pt;0pt;0pt;0pt;0pt;0pt;0pt"
        .ListWidth = 275
        .ListRows = 15
...
Dim rsno As New Recordset
    ...
    sqlno = "select porequest_no, porequest_period, isnull(fix_cls,'0') fix_cls " & _
            "from PORequest_Master where isnull(others_cls,'1')='1' and isnull(fix_cls,'0')='1'" & _
...
colSeqNo = 16
    With grid
        ...
        .ColWidth(ColCodeItem) = 1300
        .ColWidth(ColDesc) = 1500
        '.ColHidden(4) = True
        ...
        .ColHidden(ColLot) = True
        .ColHidden(ColQtyBox) = True
        ...
        .ColDataType(ColDelDate) = flexDTDate
...
Dim nextperiod As Date, endtgl As Integer, endperiod As Date, tglperiod As Date
    ...
    sqlitem = "select a.*, (select description from unit_cls uc where uc.unit_cls= a.unit_cls ) unit_desc,  (select description from curr_cls where curr_cls.Curr_cls= a.Currency_Code) Curr_desc  From ( " & _
              ...
              ",0) RemainingQty, " & _
              "prm.Department_Cls, dc.Description Department, prd.ReqDelivery_Date, pod.PORequest_No, pod.POReq_SeqNo, " & _
              "pod.Currency_Code, isnull(pod.Price,0) Price, prd.Class " & _
              "from PurchaseOrder_Detail pod " & _
              "left outer join PORequest_Detail prd on prd.PORequest_No = pod.PORequest_No and prd.POReq_Seqno = pod.POReq_SeqNo " & _
    ...
    sqlitem = sqlitem & _
              ...
              "         inner join PurchaseOrder_Master pom on pom.po_no = pod.po_no " & _
              "         where (prd.item_name=pod.item_name or pod.item_code = prd.item_code) and pod.porequest_no = '" & Trim(cborequestno.Text) & "' " & _
              "         and pod.POReq_Seqno = prd.POReq_Seqno and isnull(pom.others_cls,'0')='1') ,0) totalPOQty, " & _
              "isnull(prd.qty,0) - isnull( (select sum(qty) qty from PurchaseOrder_Detail pod " & _
              "                             inner join PurchaseOrder_Master pom on pom.po_no = pod.po_no " & _
              "                             where (pod.item_code = prd.item_code or pod.item_name=pod.item_name) and pod.porequest_no = '" & Trim(cborequestno.Text) & "' " & _
              "                             and pod.POReq_Seqno = prd.POReq_Seqno and isnull(pom.others_cls,'0')='1') " & _
              ",0) RemainingQty, " & _
              "prm.Department_Cls, dc.Description Department, prd.ReqDelivery_Date, prd.PORequest_No, prd.POReq_Seqno, " & _
    ...
    If Not (RsItem.BOF And RsItem.EOF) Then
        ...
        Do While Not RsItem.EOF
          ...
          .Rows = .Rows + 1
            ...
            .TextMatrix(i, ColDelDate) = Format(RsItem!ReqDelivery_Date, "MMM dd yyyy") ' IIf(IsNull(RsItem("ReqDelivery_Date")), "", Format(RsItem("ReqDelivery_Date1"), "MMM dd yyyy"))

            .TextMatrix(i, ColRemaining) = IIf(IsNull(RsItem("RemainingQty")), 0, Format(RsItem("RemainingQty"), "#,##0.#0"))
            
            .Cell(flexcpBackColor, i, ColCurr) = &HFFFFFF
            .TextMatrix(i, ColCurr) = is_null(RsItem("currency_code")) '
            
            
            .Cell(flexcpBackColor, i, ColPrice) = &HFFFFFF
...
Dim g As Integer
    ...
    Do While Not rsGrid.EOF
        For g = 1 To .Rows - 1
            If (Trim(.TextMatrix(g, ColCodeItem)) = Trim(rsGrid("Item_Code")) Or Trim(.TextMatrix(g, ColDesc)) = Trim(rsGrid("Item_name"))) And Trim(.TextMatrix(g, 1)) = Trim(rsGrid("PORequest_No")) And Trim(.TextMatrix(g, ColReqSeqNo)) = CStr(rsGrid("POReq_SeqNo")) Then
                ubahgrid = True
...
Sub BrowseAtas()
    ...
    If Not (RS.BOF And RS.EOF) Then
        ...
        'cboRequestNo = RTrim(Get_Field("select PORequest_No From PurchaseOrder_Detail WHERE Po_No='" & cbopono & "'", 0))
...
Dim banding As Double
    a = 0
    With grid
        If Col = ColOrder Or Col = ColPrice Then   'ORDER QTY
            If .TextMatrix(Row, Col) = "" Then .TextMatrix(Row, Col) = 0
            If IsNumeric(.TextMatrix(Row, Col)) = False Then .TextMatrix(Row, Col) = 0
            If CDbl(.TextMatrix(Row, Col)) > 9999999.99 Then LblErrMsg = DisplayMsg(4045) & " 9,999,999.99": .SetFocus: Exit Sub   '"Quantity must be lower or equal than 9,999,999.99"
            .TextMatrix(Row, Col) = Format(.TextMatrix(Row, Col), "#,##0.#0")
        End If
        
        If Col = 0 Or Col = ColOrder Or Col = ColPrice Then
           .TextMatrix(Row, ColAmount) = CDbl(IS_NOL((.TextMatrix(Row, ColOrder)))) * CDbl(IS_NOL(.TextMatrix(Row, ColPrice)))
        ...
        If Col = ColOrder Then
          ...
          End If
            If (banding) < IS_NOL(CDbl(.TextMatrix(Row, ColOrder))) Then
                'Perbandingan tidak d aktivkan.. order boleh lebih dari Request '10 janurai 2009
...
Private Sub Grid_BeforeEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    ...
    Else
        If Col <> 0 And Col <> ColOrder And Col <> colRemark Then
            ' And Col <> ColPrice And ColCurr <> Col
            Cancel = True
       End If
    End If
End Sub
                    ...
                    For i = 1 To .Rows - 1
                        If .Cell(flexcpChecked, i, 0) = flexChecked Then
                            ...
                            ElseIf CDbl(.TextMatrix(i, ColRemaining)) < 0 Then
                                ...
                                .SelectionMode = flexSelectionFree
                                .Col = ColRemaining: .Row = i: actrow = i
                                .SetFocus
                                LblErrMsg = DisplayMsg(4045) & " Qty Remaining" '"Quantity must be lower or equal than Qty Remaining"
                                Exit Sub
                                
                            ElseIf (.TextMatrix(i, ColCurr)) = "" Then
                               
                                .SelectionMode = flexSelectionFree
                                .Col = ColCurr: .Row = i: actrow = i
                                .SetFocus
                                LblErrMsg = DisplayMsg(1028)
                                Exit Sub
                            ElseIf (.TextMatrix(i, ColPrice)) = "" Or CDbl(.TextMatrix(i, ColPrice)) = 0 Then
                               
                                .SelectionMode = flexSelectionFree
                                .Col = ColPrice: .Row = i: actrow = i
                                .SetFocus
                                LblErrMsg = DisplayMsg(1026)
                                Exit Sub
            ...
            End If
                    ...
                    'UPDATE DETAIL
                        ...
                        If .Cell(flexcpChecked, i, 0) = flexChecked Then
                            ...
                            rsGrid("item_nAME") = Trim(.TextMatrix(i, ColDesc))
                            If .TextMatrix(i, ColCodeItem) <> "" Then
                                rsGrid("ItEM_cODE") = Trim(.TextMatrix(i, ColCodeItem))
                            End If
...
'                     "rtrim(tm.contact_person) contact_person, isnull(rtrim(tm.telephone),'') Supplierphone, isnull(rtrim(tm.Fax),'') SupplierFax, pom.POPayment_code, pom.POPayment_Days, pom.POPayment_Terms, " & _
...
'                     "rtrim(cp.phone2) cphone2, rtrim(cp.fax) cfax, rtrim(cp.PO_position) po_position, rtrim(cp.PO_person) po_person, " & _
'                     "rtrim(cp.POAcknowledge_Person) POAcknowledge_Person, rtrim(cp.POAcknowledge_Position) POAcknowledge_Position, " & _
'                     "rtrim(cp.POApproved_Person) POApproved_Person, rtrim(cp.POApproved_Position) POApproved_Position, tm.Trade_Cls, tm.Country_Cls, rtrim(prd.Department_cls) Department_Cls " & _
...
SqlRpt = " Select POM.Po_No, POM.Po_Date,POM.delivery_Date,PRD.PoRequest_No,PRM.PersonInCharge_Cls,PIC.Description, " & _
            ...
            vbLf & " POD.Item_code,POD.Price,POD.Qty,POD.Amount,isnull(IM.Item_Name,POD.Item_Name), " & _
            vbLf & " POD.Unit_Cls,U.Description Unit,POD.Currency_Code,C.Description Currency," & _
            vbLf & "' ' Ref,' ' ShipVia,POM.Remarks comments "
...
SqlRpt = SqlRpt & _
            ...
            vbLf & " left Join Trade_Master TM on POM.Supplier_Code=TM.Trade_Code " & _
            vbLf & " left Join Item_Master IM on POD.Item_Code=IM.Item_Code " & _
            ...
            vbLf & " where pom.po_no = '" & Trim(txtPoNo.Text) & "' and pom.others_cls = '1' and pom.period is null " & _
...
Private Sub updateMaster(Flag As Boolean)
    ...
    Dim rs_Master As New ADODB.Recordset
        sQl_Master = "select * from PurchaseOrder_Master where PO_No = '" & Trim$(txtPoNo.Text) & "' and others_cls = '1' and period is null"
