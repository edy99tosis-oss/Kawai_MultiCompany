...
Begin VB.Form FrmBC27LoadBarangLokal 
   BackColor       =   &H00FDDFE3&
   Caption         =   "Load Barang"
   ClientHeight    =   8655
   ClientLeft      =   120
   ClientTop       =   450
   ClientWidth     =   10020
   Icon            =   "FrmBC27LoadBarangLokal.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   8655
   ScaleWidth      =   10020
   ...
   End
End
Attribute VB_Name = "FrmBC27LoadBarangLokal"
...
Dim tampung As String, DokAsal As String, NoDok As String, KPPBC As String, NoAju As String, bteColCIFRupiah As String
Dim Tgl As Date
...
Set RS = New ADODB.Recordset
    ...
    KoneksiMysql
            ...
            strSQL = " SELECT a.id, KODE_DOKUMEN, KODE_KANTOR, Nomor_aju, nomor_daftar, tanggal_daftar, " & vbCrLf & _
                     ...
                     " on a.kode_status=b.kode_status where b.kode_dokumen='40' and a.kode_status='80' and nomor_aju not like '050823%' "
...
'GridBarang = 1
    If gridBarang <> "" Then
        frmBC27BrowseBarang.txtDokumenAsalLokal = DokAsal
        frmBC27BrowseBarang.txtNoLokal = NoDok
        frmBC27BrowseBarang.dtpTglLokal = Tgl
        frmBC27BrowseBarang.txtKodeBarangLokal = gridBarang.TextMatrix(gridBarang.Row, bteColKode)
        frmBC27BrowseBarang.txtUraianBarangLokal = gridBarang.TextMatrix(gridBarang.Row, bteColUraian)
        frmBC27BrowseBarang.txtMerkLokal = gridBarang.TextMatrix(gridBarang.Row, bteColMerk)
        frmBC27BrowseBarang.txtTipeLokal = gridBarang.TextMatrix(gridBarang.Row, bteColTipe)
        frmBC27BrowseBarang.txtUkuranLokal = gridBarang.TextMatrix(gridBarang.Row, bteColUkuran)
        frmBC27BrowseBarang.txtSpfLainLokal = gridBarang.TextMatrix(gridBarang.Row, bteColSPF)
        frmBC27BrowseBarang.txtSatuanLokal = gridBarang.TextMatrix(gridBarang.Row, bteColKodeSatuan)
        frmBC27BrowseBarang.txtJumlahSatuanLokal = gridBarang.TextMatrix(gridBarang.Row, bteColJumlahSatuan)
        frmBC27BrowseBarang.txtHargaPenyerahanLokal = Format(gridBarang.TextMatrix(gridBarang.Row, bteColHarga), "#,0.00")
        frmBC27BrowseBarang.txtUrutKeLokal = gridBarang.TextMatrix(gridBarang.Row, bteColSeri)
        frmBC27BrowseBarang.txtNoAjuLokal = NoAju
        frmBC27BrowseBarang.txtKPPBCLokal = KPPBC
        frmBC27BrowseBarang.dtpTglLokal = Tgl
        FrmBC27LoadBarangLokal.Hide
    End If
End Sub
...
up_GridBarang
    ...
    strSQL = " SELECT Seri_barang, Kode_barang, uraian, merk, tipe, ukuran, spesifikasi_lain, Jumlah_satuan, kode_satuan, " & vbCrLf & _
             " harga_penyerahan, CIF, CIF_Rupiah FROM tpbdb.tpb_barang where id_header='" & tampung & "'"
