...
Begin VB.Form F_DetailSalesReport 
   BackColor       =   &H00FDDFE3&
   ...
   ClientTop       =   435
   ClientWidth     =   7680
   Icon            =   "F_DetailSalesReport.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   End
End
Attribute VB_Name = "F_DetailSalesReport"
...
Private Sub Command1_Click()
    ...
    If lblcust = strAll Then
        sql = " select (invoice_master.cust_code)as sebango,trade_name,invoice_master.invoice_no,invoice_master.invoice_date, " & _
            ...
            " and fix_cls=1 " & _
            " and invoice_master.invoice_date end >= '" & Format(MDate, "YYYY-MM-DD") & "' " & _
            " and invoice_master.invoice_date end <= '" & Format(MDate1, "YYYY-MM-DD") & "' " & _
...
Private Sub command2_Click()
    ...
    sql = "select  1 HakPrice, rtrim(invm.cust_code) cust_code, rtrim(invm.invoice_no) invoice_no, invm.invoice_date, " & _
          vbLf & "rtrim(invd.po_no) po_no, rtrim(invd.item_code) item_code, invd.qty, invd.price, invd.amount, invd.currency_code, " & _
          vbLf & "rtrim(tm.trade_name) trade_name, tm.country_cls, case tm.country_cls when '0' then 'Local' else 'Export' end country_desc, case tm.country_cls when '1' then 0 else ppn end ppn, " & _
          vbLf & "rtrim(im.makeritem_code) makeritem_code, rtrim(im.item_name) item_name, rtrim(cc.description) curr_desc, " & _
          vbLf & "rtrim(cp.company_name) company_name, rtrim(cp.address1) address1, rtrim(cp.address2) address2, " & _
          vbLf & "rtrim(cp.province) province, rtrim(cp.city) city, rtrim(cp.postal_code) postal_code, rtrim(cp.phone1) phone1, rtrim(cp.phone2) phone2, rtrim(cp.fax) fax, " & _
          vbLf & "case when om.nocommercial_cls = 0 then 0 else invd.amount end nocommercial, " & _
          ...
          vbLf & "rtrim(invm.pebno) pebno, pebdate " & _
          ...
          vbLf & "inner join item_master im on invd.item_code = im.item_code " & _
          ...
          vbLf & "and invm.invoice_date <= '" & Format(MDate1, "yyyy-MM-dd") & "' "
    
    If lblcust <> strAll Then
        
        sql = sql & _
              vbLf & "and invm.cust_code = '" & cboCust & "' "
        
    End If
    
    sql = sql & _
          vbLf & "order by tm.country_cls, invd.currency_code, invm.cust_code, invm.invoice_no, om.nocommercial_cls, im.group_cls, invd.makeritem_code"
    ...
    If Not rsRpt.EOF Then
        ...
        sqlprint2 = "select b.country_cls, b.cust_code, b.currency_code, b.curr_desc, sum(b.amount) amount, sum(b.nocommercial) nocommercial, sum(total) total, sum(total_rate) total_rate, sum(ppn) ppn " & _
                    vbLf & "from ( " & _
                    ...
                    vbLf & ") b group by b.country_cls, b.cust_code, b.currency_code, b.curr_desc "
        
        If rsRpt1.State <> adStateClosed Then rsRpt1.Close
        rsRpt1.Open sqlprint2, Db, adOpenDynamic
        report.OpenSubreport("summary_cust").Database.Tables(1).SetDataSource rsRpt1, 3
        
         sqlprint3 = "select b.country_cls, b.currency_code, b.curr_desc, sum(b.amount) amount, sum(b.nocommercial) nocommercial, sum(total) total, sum(total_rate) total_rate, sum(ppn) ppn " & _
                     vbLf & "from ( " & _
                     vbLf & "select a.country_cls, a.cust_code, a.invoice_no, a.currency_code, a.curr_desc, sum(a.amount) amount, sum(a.nocommercial) nocommercial, " & _
                     vbLf & "sum(a.amount) - sum(a.nocommercial) total, (sum(a.amount) - sum(a.nocommercial)) * ppn_rate total_rate, " & _
                     vbLf & "case when a.country_cls = 1 then 0 else case when a.ppn_value = 0 then 0 else " & _
        ...
        rsrpt2.Open sqlprint3, Db, adOpenDynamic
        report.OpenSubreport("summary_country").Database.Tables(1).SetDataSource rsrpt2, 3
