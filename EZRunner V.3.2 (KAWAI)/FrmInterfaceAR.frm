...
Begin VB.Form FrmInterfaceAR 
   BackColor       =   &H00FDDFE3&
   Caption         =   "AR Interface"
   ClientHeight    =   10680
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   15120
   Icon            =   "FrmInterfaceAR.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   10680
   ScaleWidth      =   15120
   ...
   End
End
Attribute VB_Name = "FrmInterfaceAR"
...
Private Sub Cmd_Save_Click()
    ...
    IFPart = App.path & "\IFData" & "\IF_AR_" & Trim(cboCust) & "_" & Format(InvTo, "yyyyMMdd")
    ...
    Do While XData <= grid.Rows - 1
        ...
        ListOfData = ListOfData & _
                            ...
                            CDbl(grid.TextMatrix(Y, 18)) & vbTab & _
    ...
    'adoStreamOut.SaveToFile IFPart & "_UTF-8.txt", adSaveCreateOverWrite
    adoStreamOut.SaveToFile IFPart1 & "520FID04.TXT", adSaveCreateOverWrite
    Kill (IFPart & ".txt")

    PBar.Visible = False
    
    LocalPath = IFPart1 & "520FID04.TXT"
    
    Shell IFPart1 & "FTPuploadAR.bat", vbHide
    'UploadFTP
    Sleep 350

    LblErrMsg = "Export AR Data Success !"
...
'                            "  SELECT  JournalType ,  " & vbCrLf & _
'                            "          TransCode ,  " & vbCrLf & _
'                            "          CASE TransCode  " & vbCrLf & _
'                            "            WHEN 'AR' THEN 'X'  " & vbCrLf & _
'                            "            ELSE ''  " & vbCrLf & _
'                            "          END PostingControl ,  " & vbCrLf & _
'                            "          '' Ledger ,  " & vbCrLf & _
'                            "          'C520' Company_Code ,  " & vbCrLf & _
'                            "          '' Calculate_Tax_Auto ,  " & vbCrLf & _
'                            "          Invoice.PostingDate Document_Date ,  " & vbCrLf
'
'    StrSearch = StrSearch + "          Invoice.PostingDate Posting_Date ,  " & vbCrLf & _
'                            "          'I1' Document_Tpye ,  " & vbCrLf & _
'                            "          COALESCE(CM.SAP_Curr, '') Currency ,  " & vbCrLf & _
'                            "          '' Exchenge_Rate ,  " & vbCrLf & _
'                            "          'AR Doc.' Reference ,  " & vbCrLf & _
'                            "          Invoice.Invoice_No Doc_Header ,  " & vbCrLf & _
'                            "          '' Tradding_PartNer ,  " & vbCrLf & _
'                            "          LI.Posting_Key ,  " & vbCrLf & _
'                            "          Li.Account_No ,  " & vbCrLf & _
'                            "          '' GLIndicator ,  " & vbCrLf & _
'                            "          '' AssetTrans ,  " & vbCrLf
'
'    StrSearch = StrSearch + "          '' RecAccount ,  " & vbCrLf & _
'                            "          CASE WHEN TransCode = 'AR' THEN SalesAmount + TaxAmount  " & vbCrLf & _
'                            "               WHEN TransCode = 'SL' THEN SalesAmount  " & vbCrLf & _
'                            "               WHEN TransCode = 'ST' THEN TaxAmount  " & vbCrLf & _
'                            "          END Amount, '' Local_Amount, '' Group_Currency, '' Global_Curr_Comp,  " & vbCrLf & _
'                            "          LI.Tax_Code, '' TaxType,LI.Cost_Center, LI.Profit_Center, '' Buss_Area, '' Trad_Partner,  " & vbCrLf & _
'                            "          '' Partner_Profit, '' PayTerm, '' FSITE, '' PayBlock, '' PayRef, '' PayMeth, '' Baseline,  " & vbCrLf & _
'                            "          '' Note_IssueDate, '' WBS_elemen, '' OrderNo, '' ProductCode, '' Fin_Plan, '' StartDate,  " & vbCrLf & _
'                            "          '' Ref1, '' Ref2, '' Ref3, '' CentralBank, '' Suppl_Country, '' HouseBank, '' Assets_Val,  " & vbCrLf & _
'                            "          '' Qty, '' UnitOfQty, '' Segment, '' SortKey, 'AR Doc.' Item_Text, '' Trans_Type, '' Buss_Partner  " & vbCrLf & _
'                            "  FROM    LinkInterface LI  " & vbCrLf
'
'    StrSearch = StrSearch + "          LEFT JOIN ( SELECT  InvM.Cust_Code ,  " & vbCrLf & _
...
'                            "                              SUM(COALESCE(InvD.Price, 0) * COALESCE(InvD.Qty, 0)) SalesAmount ,  " & vbCrLf & _
'                            "                              COALESCE(InvM.PPH, 0) + COALESCE(InvM.PPN, 0) TaxAmount  " & vbCrLf & _
'                            "                      FROM    Invoice_detail InvD  " & vbCrLf & _
'                            "                              LEFT JOIN Invoice_Master InvM ON InvD.Invoice_No = InvM.Invoice_No  " & vbCrLf & _
'                            "                      WHERE   Invoice_Date BETWEEN @StartDate  " & vbCrLf & _
'                            "                                           AND     @EndDate  " & vbCrLf & _
'                            "                           AND InvM.Cust_Code=@Customer                                           " & vbCrLf
'
'    StrSearch = StrSearch + "                      GROUP BY InvM.Cust_Code ,  " & vbCrLf & _
'                            "                              InvD.Invoice_No ,  " & vbCrLf & _
'                            "                              InvM.Invoice_Date ,  " & vbCrLf & _
'                            "                              COALESCE(InvD.Currency_Code, '') ,  " & vbCrLf & _
'                            "                              InvM.PPH ,  " & vbCrLf & _
'                            "                              InvM.PPN  " & vbCrLf & _
'                            "                    ) Invoice ON LI.Trade_Code = Invoice.Cust_Code  " & vbCrLf & _
'                            "                                 AND LI.Curr_Code = Invoice.Currency  " & vbCrLf & _
'                            "          LEFT JOIN Curr_Mapping CM ON CM.EZRCurr = Invoice.Currency  " & vbCrLf & _
'                            "          WHERE   " & vbCrLf & _
'                            "            CASE WHEN TransCode = 'AR' THEN SalesAmount + TaxAmount  " & vbCrLf
'
'    StrSearch = StrSearch + "                 WHEN TransCode = 'SL' THEN SalesAmount  " & vbCrLf & _
'                            "                 WHEN TransCode = 'ST' THEN TaxAmount  " & vbCrLf & _
'                            "            END >0  " & vbCrLf & _
'                            "  " & vbCrLf & _
'                            " UNION ALL " & vbCrLf & _
...
'                            "            ELSE ''  " & vbCrLf
'
'    StrSearch = StrSearch + "          END PostingControl ,  " & vbCrLf & _
'                            "          '' Ledger ,  " & vbCrLf & _
'                            "          'C520' Company_Code ,  " & vbCrLf & _
'                            "          '' Calculate_Tax_Auto ,  " & vbCrLf & _
'                            "          Invoice.PostingDate Document_Date ,  " & vbCrLf & _
'                            "          Invoice.PostingDate Posting_Date ,  " & vbCrLf & _
'                            "          'I1' Document_Tpye ,  " & vbCrLf & _
'                            "          COALESCE(CM.SAP_Curr, '') Currency ,  " & vbCrLf & _
'                            "          '' Exchenge_Rate ,  " & vbCrLf & _
'                            "          'AR Doc.' Doc_Header ,  " & vbCrLf & _
'                            "          Invoice.Invoice_No Reference ,  " & vbCrLf
'
'    StrSearch = StrSearch + "          '' Tradding_PartNer ,  " & vbCrLf & _
'                            "          LI.Posting_Key ,  " & vbCrLf & _
'                            "          Li.Account_No ,  " & vbCrLf & _
'                            "          '' GLIndicator ,  " & vbCrLf & _
'                            "          '' AssetTrans ,  " & vbCrLf & _
...
'                            "               WHEN TransCode = 'ST' THEN TaxAmount  " & vbCrLf & _
'                            "          END Amount, '' Local_Amount, '' Group_Currency, '' Global_Curr_Comp,  " & vbCrLf & _
'                            "          LI.Tax_Code, '' TaxType,LI.Cost_Center, LI.Profit_Center, '' Buss_Area, '' Trad_Partner,  " & vbCrLf
'
'    StrSearch = StrSearch + "          '' Partner_Profit, '' PayTerm, '' FSITE, '' PayBlock, '' PayRef, '' PayMeth, '' Baseline,  " & vbCrLf & _
'                            "          '' Note_IssueDate, '' WBS_elemen, '' OrderNo, '' ProductCode, '' Fin_Plan, '' StartDate,  " & vbCrLf & _
'                            "          '' Ref1, '' Ref2, '' Ref3, '' CentralBank, '' Suppl_Country, '' HouseBank, '' Assets_Val,  " & vbCrLf & _
'                            "          '' Qty, '' UnitOfQty, '' Segment, '' SortKey, 'AR Doc.' Item_Text, '' Trans_Type, '' Buss_Partner  " & vbCrLf & _
'                            "  FROM    LinkInterface LI  " & vbCrLf & _
'                            "          LEFT JOIN ( SELECT  InvM.Cust_Code ,  " & vbCrLf & _
'                            "                              InvD.Invoice_No ,  " & vbCrLf & _
'                            "                              CONVERT(VARCHAR(8), InvM.Invoice_Date, 112) PostingDate ,  " & vbCrLf & _
'                            "                              COALESCE(InvD.Currency_Code, '') Currency ,  " & vbCrLf & _
'                            "                              SUM(COALESCE(InvD.Price, 0) * COALESCE(InvD.Qty, 0)) SalesAmount ,  " & vbCrLf & _
'                            "                              COALESCE(InvM.PPH, 0) + COALESCE(InvM.PPN, 0) TaxAmount  " & vbCrLf
'
'    StrSearch = StrSearch + "                      FROM    Invoice_detail InvD  " & vbCrLf & _
'                            "                              LEFT JOIN Invoice_Master InvM ON InvD.Invoice_No = InvM.Invoice_No  " & vbCrLf & _
'                            "                      WHERE   Invoice_Date BETWEEN @StartDate  " & vbCrLf & _
'                            "                                           AND     @EndDate  " & vbCrLf & _
'                            "                           AND InvM.Cust_Code=@Customer                                           " & vbCrLf & _
'                            "                      GROUP BY InvM.Cust_Code ,  " & vbCrLf & _
'                            "                              InvD.Invoice_No ,  " & vbCrLf & _
'                            "                              InvM.Invoice_Date ,  " & vbCrLf & _
'                            "                              COALESCE(InvD.Currency_Code, '') ,  " & vbCrLf & _
'                            "                              InvM.PPH ,  " & vbCrLf & _
'                            "                              InvM.PPN  " & vbCrLf
'
'    StrSearch = StrSearch + "                    ) Invoice ON LI.Trade_Code = '0000' " & vbCrLf & _
'                            "                                 AND LI.Curr_Code = Invoice.Currency  " & vbCrLf & _
'                            "          LEFT JOIN Curr_Mapping CM ON CM.EZRCurr = Invoice.Currency  " & vbCrLf & _
'                            "          WHERE   " & vbCrLf & _
'                            "            CASE WHEN TransCode = 'AR' THEN SalesAmount + TaxAmount  " & vbCrLf & _
...
'                            "                 WHEN TransCode = 'ST' THEN TaxAmount  " & vbCrLf & _
'                            "            END >0  " & vbCrLf & _
'                            "            AND NOT EXISTS (SELECT * FROM LinkInterface  " & vbCrLf & _
'                            "                   WHERE Trade_Code=@Customer AND Curr_Code=Currency) " & vbCrLf & _
'                            "  ORDER BY Invoice.Invoice_No ,  " & vbCrLf
'
'    StrSearch = StrSearch + "          TransCode  " & vbCrLf & _
'                            "  "
...
StrSearch = StrSearch + "    SELECT  JournalType ,    " & vbCrLf & _
                        "            TransCode ,    " & vbCrLf & _
                        "            CASE TransCode    " & vbCrLf & _
                        "              WHEN 'AR' THEN 'X'    " & vbCrLf & _
                        "              ELSE ''    " & vbCrLf & _
                        ...
                        "            '' Calculate_Tax_Auto ,    " & vbCrLf & _
                        "            Invoice.PostingDate Document_Date ,               " & vbCrLf & _
                        "            Invoice.PostingDate Posting_Date ,    "

StrSearch = StrSearch + "            'I4' Document_Tpye ,    " & vbCrLf & _
                        "            COALESCE(CM.SAP_Curr, '') Currency ,    " & vbCrLf & _
                        "            '' Exchenge_Rate ,    " & vbCrLf & _
                        "            coalesce(Voucher_no,'') /*'AR Doc.'*/ Reference ,    " & vbCrLf & _
                        "            Invoice.Invoice_No Doc_Header ,    " & vbCrLf & _
                        "            '' Tradding_PartNer ,    " & vbCrLf & _
                        "            LI.Posting_Key ,    " & vbCrLf & _
                        "            CASE WHEN TransCode='AR' THEN Coalesce(TRM.SAP_Code,TRM.Trade_Code) ELSE  Li.Account_No END Account_No,    " & vbCrLf & _
                        "            '' GLIndicator ,    " & vbCrLf & _
                        "            '' AssetTrans ,              '' RecAccount ,    " & vbCrLf & _
                        "            CASE WHEN TransCode = 'AR' THEN SalesAmount + TaxAmount    "

StrSearch = StrSearch + "                 WHEN TransCode = 'SL' THEN SalesAmount    " & vbCrLf & _
                        "                 WHEN TransCode = 'ST' THEN TaxAmount    " & vbCrLf & _
                        "            END Amount, '' Local_Amount, '' Group_Currency, '' Global_Curr_Comp,    " & vbCrLf & _
                        "            CASE WHEN TransCode='AR' THEN '' Else LI.Tax_Code end  Tax_Code, '' TaxType,LI.Cost_Center, LI.Profit_Center, '' Buss_Area, '' Trad_Partner,    " & vbCrLf & _
                        "            '' Partner_Profit, '' PayTerm, '' FSITE, '' PayBlock, '' PayRef, '' PayMeth, '' Baseline,    " & vbCrLf & _
                        "            '' Note_IssueDate, '' WBS_elemen, '' OrderNo, '' ProductCode, '' Fin_Plan, '' StartDate,    " & vbCrLf & _
                        "            '' Ref1, '' Ref2, '' Ref3, '' CentralBank, '' Suppl_Country, '' HouseBank, '' Assets_Val,    " & vbCrLf & _
                        "            '' Qty, '' UnitOfQty, '' Segment, '' SortKey,  replace(coalesce(Voucher_description,''),char(13),'') /*'AR Doc.'*/ Item_Text, '' Trans_Type, '' Buss_Partner,Interface_Cls     " & vbCrLf & _
                        "    FROM    ( " & vbCrLf & _
                        "     " & vbCrLf & _
                        "               Select * From (  "

StrSearch = StrSearch + "               Select B.cust_Code,C.*,0 orderby From ( " & vbCrLf & _
                        "                   Select Coalesce(A.Trade_Code,'0000')Link,Cust_Code From (Select Distinct Cust_Code From Invoice_Master Where Invoice_Date between  @StartDate and @EndDate )AP Left Join  " & vbCrLf & _
                        "                   (Select * From LinkInterface Where JournalType='AR' And TransCode='AR')A On ap.Cust_Code=a.Trade_Code " & vbCrLf & _
                        "               )B Left JOin (Select * From LinkInterface Where JournalType='AR' And TransCode='AR')C On B.Link=C.Trade_Code " & vbCrLf & _
                        "  " & vbCrLf & _
                        "               union ALL " & vbCrLf & _
                        "  " & vbCrLf & _
                        "               Select B.Cust_Code,C.*,1 orderBy From ( " & vbCrLf & _
                        "                   Select Coalesce(A.Trade_Code,'0000')Link,Cust_Code From (Select Distinct Cust_Code From Invoice_Master Where Invoice_Date between  @StartDate and @EndDate )AP Left Join  " & vbCrLf & _
                        "                   (Select * From LinkInterface Where JournalType='AR' And TransCode='SL')A On ap.Cust_Code=a.Trade_Code " & vbCrLf & _
                        "               )B Left JOin (Select * From LinkInterface Where JournalType='AR' And TransCode='SL')C On B.Link=C.Trade_Code "

StrSearch = StrSearch + "  " & vbCrLf & _
                        "               Union ALL " & vbCrLf & _
                        "  " & vbCrLf & _
                        "               Select B.Cust_Code,C.*,2 orderBy From ( " & vbCrLf & _
                        "                   Select Coalesce(A.Trade_Code,'0000')Link,Cust_Code From (Select Distinct Cust_Code From Invoice_Master Where Invoice_Date between  @StartDate and @EndDate )AP Left Join  " & vbCrLf & _
                        "                   (Select * From LinkInterface Where JournalType='AR' And TransCode='ST')A On ap.Cust_Code=a.Trade_Code " & vbCrLf & _
                        "               )B Left JOin (Select * From LinkInterface Where JournalType='AR' And TransCode='ST')C On B.Link=C.Trade_Code " & vbCrLf & _
                        "               )D  " & vbCrLf & _
                        "  " & vbCrLf & _
                        "    )LI    " & vbCrLf & _
                        "            LEFT JOIN ( SELECT  InvM.Cust_Code ,    "

StrSearch = StrSearch + "                                InvD.Invoice_No ,CONVERT(VARCHAR(8), InvM.Invoice_Date, 112) PostingDate ,    " & vbCrLf & _
                        "                                COALESCE(InvD.Currency_Code, '') Currency ,    " & vbCrLf & _
                        "                                SUM(COALESCE(InvD.Price, 0) * COALESCE(InvD.Qty, 0)) SalesAmount ,    " & vbCrLf & _
                        "                                COALESCE(InvM.PPN, 0) TaxAmount,Interface_Cls, voucher_no, voucher_description     " & vbCrLf & _
                        "                        FROM    Invoice_detail InvD    " & vbCrLf & _
                        "                                LEFT JOIN Invoice_Master InvM ON InvD.Invoice_No = InvM.Invoice_No    " & vbCrLf & _
                        "                                LEFT JOIN OrderEntry_Detail OE on Invd.po_no = OE.Po_no AND Invd.seq_no = OE.seq_no and Invd.Item_code = OE.Item_Code   " & vbCrLf & _
                        "                                LEFT JOIN OrderEntry_Master OM ON OE.PO_No = OM.PO_No " & vbCrLf & _
                        "                        WHERE   Invoice_Date BETWEEN @StartDate    " & vbCrLf & _
                        "                                             AND     @EndDate    and OM.Nocommercial_Cls ='0' " & vbCrLf & _
                        "                                                                         " & vbCrLf & _
                        "                        GROUP BY InvM.Cust_Code ,    " & vbCrLf & _
                        "                                InvD.Invoice_No ,                                  InvM.Invoice_Date ,    "

StrSearch = StrSearch + "                                COALESCE(InvD.Currency_Code, '') ,    " & vbCrLf & _
                        "                                InvM.PPN,Interface_Cls, voucher_no, voucher_description   " & vbCrLf & _
                        "                      ) Invoice ON LI.Cust_Code = Invoice.Cust_Code    " & vbCrLf & _
                        "                                    " & vbCrLf & _
                        "            LEFT JOIN Curr_Mapping CM ON CM.EZRCurr = Invoice.Currency    " & vbCrLf & _
                        "            LEFT JOIN Trade_Master TRM ON LI.Cust_Code=TRM.Trade_Code " & vbCrLf & _
                        "            WHERE    " & vbCrLf & _
                        "              CASE WHEN TransCode = 'AR' THEN SalesAmount + TaxAmount    " & vbCrLf & _
                        "                   WHEN TransCode = 'SL' THEN SalesAmount    " & vbCrLf & _
                        "                   WHEN TransCode = 'ST' THEN TaxAmount                END >0    " & vbCrLf & _
                        IIf(cbointeface.ListIndex = 0, " and Coalesce(Interface_Cls,'0')='1' ", "and Coalesce(Interface_Cls,'0')='0'") & vbCrLf & _
                        IIf(cboCust.ListIndex = 0, "And LI.Cust_Code NOT IN " & strExcludeCustSup & " ", "And LI.Cust_Code='" & Trim(cboCust.Text) & "'") & vbCrLf & _
                        "And Invoice.Cust_Code<>'C020' "

StrSearch = StrSearch + "  ORDER BY Invoice.Invoice_No ,    " & vbCrLf & _
                        "            TransCode   "
    ...
    Do While Not RsSearch.EOF
        ...
        For XData = 2 To RsSearch.Fields.Count - 2
...
Sub Kosong()
    ...
    strSQL = "Select Trade_Code Cust_Code, Trade_Name Cust_Name" & vbCrLf & _
                  ...
                  "        WHERE (trade_cls in ('2')) --and tradeexternal_cls in ('0', '2')) or (trade_cls in ('4')) " & vbCrLf & _
                  "            ORDER BY Cust_Code " & vbCrLf
