...
'
'    ' Hanya jika belum approve
'    If Trim(grid.TextMatrix(Row, bteColApprovedDate)) = "" Then
'
'        If Trim(grid.Text) <> "" Then
'            If Not IsNumeric(grid.Text) Then
'                MsgBox "Seq No hanya boleh diisi angka.", vbExclamation
'                grid.Text = ""
'                Cancel = True
'                Exit Sub
'            End If
...
If Not (RsSerial.BOF And RsSerial.EOF) Then
        With grid
            Do While Not RsSerial.EOF
                If i = 1 Then
                    uf_ValidasiSerialNo = "Serial No Already Exist..! " & uf_ValidasiSerialNo & "Item Code :" & RTrim(RsSerial("Item_code")) & " - Date : " & Format(RsSerial("Schedule_Date"), "dd MMM yyyy")
                Else
