...
Begin VB.Form frmPOPartsScheduled 
   BackColor       =   &H00FDDFE3&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Purchase Order Scheduled (Part / Material)"
   ClientHeight    =   10920
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   15270
   Icon            =   "frmPOPartsScheduled.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   ScaleHeight     =   10920
   ScaleWidth      =   15270
   ...
   Begin VSFlex8Ctl.VSFlexGrid grid 
      ...
      FormatString    =   $"frmPOPartsScheduled.frx":0E42
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmPOPartsScheduled"
...
'            "and pom.po_date <= '" & Format(RequestDate2, "yyyy-mm-dd") & "' "
    ...
    sqlno = "select pom.PO_No from PurchaseOrder_Master pom " & _
            "where pom.others_cls = '0' and pom.period is null and pom.sheetcoil_cls = '0' " & _
            "and pom.po_date >= '" & Format(requestdate1, "yyyy-mm-dd") & "' " & _
            "and pom.po_date <= '" & Format(requestdate2, "yyyy-mm-dd") & "' "
    Set rsno = Db.Execute(sqlno)
    With CboPOnO
                ...
                With grid
                    For i = 1 To .Rows - 1
                        If .Cell(flexcpChecked, i, 0) = flexChecked Then
                            If .TextMatrix(i, ColOrder) = "0" Or .TextMatrix(i, ColOrder) = "" Then
...
Private Sub updateMaster(Flag As Boolean)
    ...
    Dim rs_Master As New ADODB.Recordset
        ...
        If Flag = True Then
            rs_Master("discount") = CDbl(TxtDisc.Text) 'Add 20090112
