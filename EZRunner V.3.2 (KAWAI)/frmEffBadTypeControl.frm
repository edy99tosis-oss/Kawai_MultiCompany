...
Begin VB.Form frmEffBadTypeControl 
   BackColor       =   &H00FDDFE3&
   Caption         =   "Defective Type Pareto Diagram"
   ClientHeight    =   4065
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   8385
   ...
   EndProperty
   Icon            =   "frmEffBadTypeControl.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   ScaleHeight     =   4065
   ScaleWidth      =   8385
   ...
   End
End
Attribute VB_Name = "frmEffBadTypeControl"
...
Private Sub cboFactory_Change()
    If cboFactory.ListIndex <> -1 Then
        ...
        Call isiCbo(CboLine, "Manufacture_Line", "Line_Code", "Line_Name", 80, 140, "Line_Code", , , " Manufacture_Code = '" & cboFactory & "' ", 0)
    Else
...
Dim sqlLine As String
    ...
    sql = "Select dt.* " & vbCrLf & _
        "From " & vbCrLf & _
            "( " & vbCrLf & _
                "Select FactoryCD = R.Supplier_Code, S.MaterialConsump_Cls, Mcc.Description, " & vbCrLf & _
                    "qty = IsNull(Sum(s.ChildRequirement_Qty), 0) " & vbCrLf & _
                "from Part_Supply S, Part_Receipt R, MaterialConsump_Cls Mcc " & vbCrLf & _
                "where S.DO_No = Convert(Char,R.Seq_No) And S.MaterialConsump_Cls= Mcc.MaterialConsump_Cls " & vbCrLf & _
                    "And R.ProductionResult_Cls = '1' And S.Supply_Cls = 'S' and isnull(rtrim(S.DO_No),'') <> '' " & vbCrLf & _
                    "And S.MaterialConsump_Cls in " & vbCrLf & _
                        "(Select distinct top 7 s2.MaterialConsump_Cls From Part_Supply S2, Part_Receipt R2 " & vbCrLf & _
                        "where S2.DO_No = Convert(Char,R2.Seq_No) And R2.ProductionResult_Cls = 1 " & vbCrLf & _
                            "And year(S2.ChildSupply_Date) = '" & Year(Period) & "' and month(S2.ChildSupply_Date) = '" & Month(Period) & "' " & vbCrLf & _
                            "And S2.Supply_Cls = 'S' and isnull(rtrim(S2.DO_No),'') <> '' " & vbCrLf & _
                            "And S2.MaterialConsump_Cls is not null " & vbCrLf & _
                            "And R2.Supplier_Code = '" & cboFactory & "' " & sqlLine & vbCrLf & _
                        ") " & vbCrLf & _
                    "And year(S.ChildSupply_Date) = '" & Year(Period) & "' and month(S.ChildSupply_Date) = '" & Month(Period) & "' " & vbCrLf & _
                    "And R.Supplier_Code = '" & cboFactory & "' " & sqlLine & vbCrLf & _
                "Group By R.Supplier_Code, S.MaterialConsump_Cls,  Mcc.Description "
    sql = sql & vbCrLf & _
                "Union " & vbCrLf & _
                "Select FactoryCD = R.Supplier_Code, MaterialConsump_Cls = 'zz', Description = 'Others', " & vbCrLf & _
                    "qty = IsNull(Sum(s.ChildRequirement_Qty), 0) " & vbCrLf & _
                "from Part_Supply S, Part_Receipt R " & vbCrLf & _
                "where S.DO_No = Convert(Char,R.Seq_No) " & vbCrLf & _
                    "And R.ProductionResult_Cls = '1' And S.Supply_Cls = 'S' and isnull(rtrim(S.DO_No),'') <> '' " & vbCrLf & _
                    "And S.MaterialConsump_Cls not in " & vbCrLf & _
                        "(Select distinct top 7 s2.MaterialConsump_Cls From Part_Supply S2, Part_Receipt R2 " & vbCrLf & _
                        "where S2.DO_No = Convert(Char,R2.Seq_No) And R2.ProductionResult_Cls = 1 " & vbCrLf & _
                            "And year(S2.ChildSupply_Date) = '" & Year(Period) & "' and month(S2.ChildSupply_Date) = '" & Month(Period) & "' " & vbCrLf & _
                            "And S2.Supply_Cls = 'S' and isnull(rtrim(S2.DO_No),'') <> '' " & vbCrLf & _
                            ...
                            "And R2.Supplier_Code = '" & cboFactory & "' " & sqlLine & vbCrLf & _
                        ") " & vbCrLf & _
                    "And S.MaterialConsump_Cls is not null " & vbCrLf & _
                    "And year(S.ChildSupply_Date) = '" & Year(Period) & "' and month(S.ChildSupply_Date) = '" & Month(Period) & "' " & vbCrLf & _
                    "And R.Supplier_Code = '" & cboFactory & "' " & sqlLine & vbCrLf & _
                "Group By R.Supplier_Code " & vbCrLf & _
            ")dt Order By dt.FactoryCD, (Case When dt.MaterialConsump_Cls = 'zz' Then -1 Else Qty End) desc"
