...
Begin VB.Form frmBC25BrowseBarangTPB 
   BackColor       =   &H00FDDFE3&
   ...
   EndProperty
   Icon            =   "frmBC25BrowseBarangTPB.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   8070
   ...
   End
End
Attribute VB_Name = "frmBC25BrowseBarangTPB"
...
Private Sub up_GridLoadAju()
    ...
    With gridAju
        While Not RS.EOF
            ...
            .TextMatrix(li_Row, colNomorAju) = Trim(RS!No_Pengajuan) 'IIf(IsNull(rs.Fields("NETTO")), 0, rs.Fields("NETTO"))
            .TextMatrix(li_Row, colSuratJalan) = IIf(IsNull(RS!SuratJalan_No), 0, Trim(RS!SuratJalan_No))
            .TextMatrix(li_Row, colTanggal) = IIf(IsNull(RS!Pengajuan_Date), 0, Trim(RS!Pengajuan_Date))
...
If gridHS.RowSel > 0 Then
    If frmBC25BrowseBarang.SSTab1.Tab = 1 Then
        frmBC25BrowseBarang.txtNoAjuImpor = txtHideNoAju
        frmBC25BrowseBarang.txtDokumenAsalImpor = "23"
        frmBC25BrowseBarang.lblDokAsalImpor = "BC 2.3"
        frmBC25BrowseBarang.txtNomorHSImpor = gridHS.TextMatrix(gridHS.RowSel, colNomorHS)
        frmBC25BrowseBarang.txtUraianBarangImpor = gridHS.TextMatrix(gridHS.RowSel, colUraian)
        frmBC25BrowseBarang.txtUrutKeImpor = gridHS.TextMatrix(gridHS.RowSel, colNomorSeri)
        frmBC25BrowseBarang.txtKPPBCImpor = gridAju.TextMatrix(gridAju.RowSel, colKodeKantor)
        frmBC25BrowseBarang.lblKPPBCImpor = gridAju.TextMatrix(gridAju.RowSel, colNamaKantor)
        
        frmBC25BrowseBarang.txtKodeBarangImpor = gridHS.TextMatrix(gridHS.RowSel, colKodeBarang)
        frmBC25BrowseBarang.txtTipeImpor = gridHS.TextMatrix(gridHS.RowSel, colTipe)
        frmBC25BrowseBarang.txtHargaCIFUSDImpor = Format(gridHS.TextMatrix(gridHS.RowSel, colCIF), "#,0.00")
        
        frmBC25BrowseBarang.txtNettoImpor = Format(gridHS.TextMatrix(gridHS.RowSel, colNetto), "#,0.00")
    '    frmBC25BrowseBarang.gd_HargaPenyerahanImpor = gridHS.TextMatrix(gridHS.RowSel, colHargaPenyerahan)
    Else
        frmBC25BrowseBarang.txtNoAjuLokal = txtHideNoAju
        frmBC25BrowseBarang.txtDokumenAsalLokal = "23"
        frmBC25BrowseBarang.lblDokAsalLokal = "BC 2.3"
        frmBC25BrowseBarang.txtNomorHSLokal = gridHS.TextMatrix(gridHS.RowSel, colNomorHS)
        frmBC25BrowseBarang.txtUraianBarangLokal = gridHS.TextMatrix(gridHS.RowSel, colUraian)
        frmBC25BrowseBarang.txtUrutKeLokal = gridHS.TextMatrix(gridHS.RowSel, colNomorSeri)
        frmBC25BrowseBarang.txtKPPBCLokal = gridAju.TextMatrix(gridAju.RowSel, colKodeKantor)
        frmBC25BrowseBarang.lblKPPBCLokal = gridAju.TextMatrix(gridAju.RowSel, colNamaKantor)
        
        frmBC25BrowseBarang.txtKodeBarangLokal = gridHS.TextMatrix(gridHS.RowSel, colKodeBarang)
        frmBC25BrowseBarang.txtTipeLokal = gridHS.TextMatrix(gridHS.RowSel, colTipe)
        frmBC25BrowseBarang.txtHargaCIFUSDLokal = Format(gridHS.TextMatrix(gridHS.RowSel, colCIF), "#,0.00")
        frmBC25BrowseBarang.txtNDPBMLokal = Format(gridHS.TextMatrix(gridHS.RowSel, colHargaPenyerahan), "#,0.00")
