...
Begin VB.Form frmRptLedgerAR 
   BackColor       =   &H00FDDFE3&
   Caption         =   "General / Sub Ledger - AR"
   ClientHeight    =   4725
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   9600
   ...
   End
End
Attribute VB_Name = "frmRptLedgerAR"
...
Dim Opening(5) As Integer, sales(5) As Integer, colFP As Integer, collection(5) As Integer, Ending(5) As Integer
...
With Cbo(0)
    ...
    sql = "select Trade_Code,Trade_Name from Trade_Master " & _
        "where Trade_Cls = 2 or trade_cls=4 " & _
        "order by Trade_Code"
...
Dim sqlM As String, sqlMAR As String
    ...
    If st = 0 Then 'Curr
        sqlM = "distinct CurrCD = ISNULL((Select Top 1 Case Currency_Code When '03' then '00' else Currency_Code End From Invoice_Detail Where Invoice_No = INV.Invoice_No),'') "
    ...
    sql = "( Select " & sqlM & "From Invoice_Master INV " & _
                "Where INV.fix_Cls ='1' And month(invoice_date)= " & Month(dt) & " and year(invoice_date) = " & Year(dt) & sqlcust & _
            " UNION " & _
            "Select " & sqlMAR & "From AR_Master INV, AR_Detail AD " & _
                "Where INV.AR_No = AD.AR_No And month(AR_Date)= " & Month(dt) & " and year(AR_Date) =" & Year(dt) & sqlcust & _
            " UNION " & _
            "Select " & sqlM & "From " & _
                "( " & _
                    "Select INV.invoice_no, " & _
                        "VAT = isnull((select PPN_IDR from fakturpajak_master " & _
                                            "where fakturpajak_no = (Select top 1 fakturPajak_no from fakturpajak_detail where invoice_no = INV.invoice_no)),0), " & _
                        "INV.amount, " & _
                        "CollectionBefore = ISNULL((Select SUM(Amount) From AR_Master AM, AR_Detail AD " & _
                                            "Where AM.AR_No = AD.AR_No And AD.Invoice_No = INV.Invoice_No " & _
                                                "And AM.AR_Date < '" & Format(dt, "yyyy-MM-01") & "'),0), " & _
                        "CollectionPPNBefore = ISNULL((Select SUM(PPN) From AR_Master AM, AR_Detail AD " & _
                                            "Where AM.AR_No = AD.AR_No And AD.Invoice_No = INV.Invoice_No " & _
                                                "And AM.AR_Date < '" & Format(dt, "yyyy-MM-01") & "'),0) "

    sql = sql & _
                    "from invoice_master INV,  trade_master c " & _
                    "where INV.cust_code = c.trade_code and INV.fix_Cls ='1' " & _
                        "and invoice_date < '" & Format(dt, "yyyy-MM-01") & "' " & sqlcust & _
                        " and (INV.Paid_Date >= '" & Format(dt, "yyyy-MM-01") & "' Or INV.Paid_Date IS NULL) " & _
                ") INV Where (Amount - CollectionBefore > 0 Or VAT - CollectionPPNBefore > 0) "
...
Dim monthRate As String
    ...
    If currRate <> "" Then currRate = Left(currRate, Len(currRate) - 1)
    Set rsrate = Nothing
End Function
...
Dim clsMRP As New clsMRP
    ...
    With excelapp
        ...
        If Option1(0).Value = True Then
            .Cells(2, 1) = "General Ledger - AR "
            kurang = 4
        Else
            .Cells(1, 1) = clsMRP.CompanyName
            .Cells(2, 1) = "Sub Ledger - AR "
            kurang = 0
        End If
        
        .Cells(3, 1) = "Period End : " & Format(tglAkhir, "dd MMM yyyy")
        .Range("A1", "A3").Font.Bold = True
        
        .Cells(5, 7 - kurang) = "Non Collected till end of Last Month"
        .Range(.Cells(5, 7 - kurang), .Cells(5, 6 + totCurrRate - kurang)).Merge
        
        .Cells(5, 7 + totCurrRate - kurang) = "Sales Invoice Current"
        .Range(.Cells(5, 7 + totCurrRate - kurang), .Cells(5, 6 + (totCurrRate * 2) - kurang)).Merge
        
        .Cells(5, 7 + (totCurrRate * 2) - kurang) = "FP Amount"
        .Cells(5, 8 + (totCurrRate * 2) - kurang) = "Collection Current Month"
        
        If totCurrRate = 1 Then
            .Cells(5, 8 + (totCurrRate * 3) - kurang) = "Total Non Collected at End Current Month"
        Else
            .Cells(5, 8 + (totCurrRate * 3) + 1 - kurang) = "Total Non Collected at End Current Month"
        ...
        .Cells(6, 7 + totCurrRate - kurang) = "Sales In Month"
        .Range(.Cells(6, 7 + totCurrRate - kurang), .Cells(6, 7 + (totCurrRate * 2) - kurang)).Merge
        
        .Cells(6, 8 + (totCurrRate * 2) - kurang) = "Collection"
        ...
        .Range(.Cells(5, 1), .Cells(7, 8 + (totCurrRate * 4) - kurang)).HorizontalAlignment = xlCenter
...
Function sqlLedger(stGeneral As Byte) As String
    If stGeneral = 1 Then
        sql = "Select Cust_Code, Trade_Name, CurrCD, " & _
        ...
        sql = sql & "Select dt.Cust_Code, dt.Trade_Name, dt.CurrCD, " & _
                        "OpeningAmount = " & _
        ...
        sql = sql & "From " & _
                    "( " & _
                        "Select IVM.Cust_Code, T.Trade_Name, IVM.Invoice_No, IVM.Invoice_Date, " & _
                            "IVM.Delivery_Date, IVM.Due_Date, Paid_Cls = ISNULL(IVM.Paid_Cls,0), Paid_Date," & _
                            "CurrCD = ISNULL((Select Top 1 Currency_Code From Invoice_Detail Where Invoice_No = IVM.Invoice_No),''), " & _
                            "InvAmount = IVM.Amount, " & _
                            "PPNIDR = ISNULL((Select top 1 PPN_IDR From FakturPajak_Master FPM, FakturPajak_Detail FPD " & _
                                            "Where FPM.FakturPajak_No = FPD.FakturPajak_No " & _
                                                "ANd FPD.Invoice_No = IVM.Invoice_No" & _
                                            "),0), "
                                
        sql = sql & _
                            "ARAmount = ISNULL((Select ISNULL(SUM(ARD.Amount),0) From AR_Master ARM, AR_Detail ARD " & _
                                            "Where ARM.AR_No = ARD.AR_No And ARD.Invoice_No = IVM.Invoice_No And Year(ARM.AR_Date) = " & Year(dt) & " And Month(ARM.AR_Date) = " & Month(dt) & "),0), " & _
                            "ARPPN = ISNULL((Select ISNULL(SUM(ARD.PPN),0) From AR_Master ARM, AR_Detail ARD " & _
                                            "Where ARM.AR_No = ARD.AR_No And ARD.Invoice_No = IVM.Invoice_No And Year(ARM.AR_Date) = " & Year(dt) & " And Month(ARM.AR_Date) = " & Month(dt) & "),0), " & _
                            "ARAmountBefore = ISNULL((Select ISNULL(SUM(ARD.Amount),0) From AR_Master ARM, AR_Detail ARD " & _
                                            "Where ARM.AR_No = ARD.AR_No And ARD.Invoice_No = IVM.Invoice_No And ARM.AR_Date < '" & Format(dt, "yyyy-MM-01") & "'),0), " & _
                            "ARPPNBefore = ISNULL((Select ISNULL(SUM(ARD.PPN),0) From AR_Master ARM, AR_Detail ARD Where ARM.AR_No = ARD.AR_No And ARD.Invoice_No = IVM.Invoice_No And ARM.AR_Date < '" & Format(dt, "yyyy-MM-01") & "' " & "),0) " & _
                        "From Invoice_Master IVM, Trade_Master T " & _
                        "Where IVM.Cust_Code = T.Trade_Code " & _
                    ") dt, " & sqlMaster(1) & _
                "Where dt.Invoice_No = Master.InvNo And dt.CurrCD <> '' " & _
                ") data " & _
            "Group By Cust_Code, Trade_Name, CurrCD " & _
            "Order By Cust_Code, Trade_Name, CurrCD "
    
    Else
    
        sql = "Select dt.Cust_Code, dt.Trade_Name, dt.Invoice_No, dt.Invoice_Date, " & _
                    "dt.Delivery_Date, dt.Due_Date, dt.Paid_Cls, dt.CurrCD, " & _
                    "OpeningAmount = " & _
                        "Case When dt.CurrCD = '03' Or (Month(Invoice_Date) = " & Month(dt) & " And Year(Invoice_Date) = " & Year(dt) & ") Then 0 " & _
                            "Else InvAmount - ARAmountBefore End, " & _
                    "OpeningPPN = " & _
        ...
        sql = sql & _
                    ...
                    ") dt, " & sqlMaster(1) & _
                "Where dt.Invoice_No = Master.InvNo And dt.CurrCD <> '' " & _
            "Order By Cust_Code, CurrCD, Invoice_No, Invoice_Date, Delivery_Date "
    End If
    sqlLedger = sql
End Function
    ...
    If Not rsLedger.EOF Then
        With excelapp
            ...
            Do While Not rsLedger.EOF
                ...
                If custCD <> Trim(rsLedger!Cust_CodE) Then
                ...
                If stGeneral = 0 Then
                    ...
                    .Cells(brs, 5) = Format(Left(rsLedger!delivery_Date, 4) & "-" & Right(rsLedger!delivery_Date, 2), "MMM yyyy")
                    .Cells(brs, 6) = IIf(IsNull(rsLedger!due_date), "", Format(rsLedger!due_date, "dd MMM yyyy"))
