...
Begin VB.Form FrmValuationPriceGroup 
   BackColor       =   &H00FDDFE3&
   Caption         =   "Valuation Price Master (By Group)"
   ClientHeight    =   10740
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   15105
   Icon            =   "FrmValuationPriceGroup.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   10740
   ScaleWidth      =   15105
   ...
   End
End
Attribute VB_Name = "FrmValuationPriceGroup"
...
Dim ColS As Integer
Dim ColGroup As Integer, ColGroupDes As Integer
...
Sub Header()
    With grid
        ...
        .TextMatrix(0, ColS) = ""
        .TextMatrix(0, ColGroup) = "Group Cls"
        .TextMatrix(0, ColGroupDes) = "Description"
        ...
        .ColWidth(ColS) = 300
        .ColWidth(ColGroup) = 1200
        .ColWidth(ColGroupDes) = 1800
        .ColWidth(cOlcost) = 1200
        .ColWidth(ColCostDes) = 1800
        ...
        .ColAlignment(ColGroup) = flexAlignLeftCenter
        .ColAlignment(ColGroupDes) = flexAlignLeftCenter
...
Sub IsiGrid()
    ...
    With grid
        ...
        Do While Not rsGrid.EOF
            ...
            .TextMatrix(.Rows - 1, ColGroup) = Trim(rsGrid("Group_Cls"))
            .TextMatrix(.Rows - 1, ColGroupDes) = Trim(rsGrid("Group_Desc"))
    ...
    With grid
        For i = 1 To .Rows - 1
            If .TextMatrix(i, 0) = "D" Then
                ...
                If tanya = vbYes Then
                    sql = "Delete From InventoryCost_Group Where Group_Cls = '" & Trim(.TextMatrix(i, ColGroup)) & "' And " & _
        ...
        If Trim(cboGroup.Text) = "" Then
            LblErrMsg = DisplayMsg(8056) '"Please Select Group Cls"
            Exit Sub
        End If
        
        cboGroup.Text = cboGroup.Text
        If cboGroup.MatchFound = False Then
            LblErrMsg = DisplayMsg(8056)  ''"Record with This Group Cls Not found"
            Exit Sub
        End If
        ...
        If Trim(cbocurr.Text) = "" Then
            LblErrMsg = DisplayMsg(4005) ' '"Please Select Currency"
            Exit Sub
        End If
        ...
        sql = "Select * From InventoryCost_Group"
        If RS.State <> adStateClosed Then RS.Close
        RS.Open sql, Db, 1, 3
        
        RS.filter = "Group_Cls = '" & Trim(cboGroup.Text) & "' " & _
        ...
        RS("Group_Cls") = Trim(cboGroup.Text)
...
Sub cektgl()
    ...
    If hapus Then
        sql = "Select * From InventoryCost_Group Where " & _
                "Group_Cls = '" & Trim(cboGroup.Text) & "' " & _
                "And Cost_Cls = '" & Trim(CboCost.Text) & "' " & _
        ...
        If Not (rsBefore.BOF And rsBefore.EOF) Then
            ...
            If Not (rsAfter.BOF And rsAfter.EOF) Then
                ...
                sql = "Update InventoryCost_Group Set End_Date = '" & TempDate & "', Last_Update = getdate(), Last_User = '" & userLogin & "' Where " & _
                      "Group_Cls = '" & rsBefore("Group_Cls") & "' " & _
                      "And Cost_Cls = '" & rsBefore("Cost_Cls") & "' " & _
                      "And Start_Date = '" & rsBefore("Start_Date") & "' "
                Db.Execute sql
        
            Else
                sql = "Update InventoryCost_Group Set End_Date = '99999999', Last_Update = getdate(), Last_User = '" & userLogin & "' Where " & _
                      "Group_Cls = '" & rsBefore("Group_Cls") & "' " & _
                      "And Cost_Cls = '" & rsBefore("Cost_Cls") & "' " & _
                      "And Start_Date = '" & rsBefore("Start_Date") & "' "
                Db.Execute sql
    ...
    With grid
        If Col = ColS Then
            If .TextMatrix(Row, ColS) = "S" Then
                cboGroup = Trim(.TextMatrix(Row, ColGroup))
                lblgroup = Trim(.TextMatrix(Row, ColGroupDes))
...
Function SqlSintaks() As String
    ...
    SqlS = "Select ICG.Group_Cls, GC.Description Group_Desc, ICG.Cost_Cls, ICM.Description Cost_Desc, Start_Date, End_Date, " & _
            ...
            "Inner Join InventoryCost_Group ICG on ICM.Cost_Cls = ICG.Cost_Cls " & _
            "Inner Join Group_Cls GC on ICG.Group_Cls = GC.Group_Cls "
    SqlS = SqlS + "Order by ICG.Group_Cls, ICG.Cost_Cls, Start_Date, Currency_Code "
...
Private Sub StartDate_Change()
    If mask.Text <> "99/99/9999" Then
        ...
        If CDate(StartDate.Value) > CDate(EndDate.Value) Then
           LblErrMsg.Caption = DisplayMsg(4068)  ' "Start Date must be lower than " & Format(EndDate, "dd MMM yyyy") 'DisplayMsg(4025)
           Exit Sub
        End If
    End If
End Sub
...
Private Sub EndDate_Change()
    ...
    If CDate(EndDate) < CDate(StartDate) Then
       LblErrMsg.Caption = DisplayMsg(4066)  ' "End Date must be higher than " & Format(StartDate, "dd MMM yyyy") 'DisplayMsg(4024)
       Exit Sub
    End If
End Sub
