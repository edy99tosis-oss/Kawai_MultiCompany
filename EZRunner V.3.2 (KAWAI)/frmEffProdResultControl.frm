...
Begin VB.Form frmEffProdResultControl 
   BackColor       =   &H00FDDFE3&
   ...
   EndProperty
   Icon            =   "frmEffProdResultControl.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   ScaleHeight     =   4215
   ...
   End
End
Attribute VB_Name = "frmEffProdResultControl"
...
Private Sub Form_Load()
  If gb_Simulation = True Then Call up_InitSimulation(Me)
    ...
    file = App.path & "\Excel\ProdResultControl.xls"
End Sub
...
Dim kondD As String, kondR As String, strLine As String, strLine2 As String
    ...
    sql = "Select dtAll.Tgl, dtAll.Factory_Code " & strLine & _
            ", QtyDP = SUM(dtAll.QtyDP), QtyR = SUM(dtAll.QtyR), diff = SUM(dtAll.QtyR - dtAll.QtyDP), " & _
            "dtAll.MaxResultDt, dtAll.MaxDailyDt, dtAll.completeCls "
    ...
    sql = sql & _
                    "QtyR = " & _
                        "isnull(( " & _
                                ...
                                "),0), " & _
                    "MaxResultDt = (Select Max(Receipt_Date) From Part_Receipt R " & _
                                    "Where Receipt_Cls = 'P1' " & _
                                        "And Supplier_Code = dtTgl.Factory_Code " & kondR & _
                                        "And Year(Receipt_Date) = " & Year(dt) & " And Month(Receipt_Date) = " & Month(dt) & _
                                    "), " & _
                    "MaxDailyDt = (Select Max(Schedule_Date) From Daily_Production dt " & _
                                    "Where Factory_Code = dtTgl.Factory_Code " & kondD & _
                                        "And Year(Schedule_Date) = " & Year(dt) & " And Month(Schedule_Date) = " & Month(dt) & _
                                    ") "
    sql = sql & _
                "From " & _
    ...
    sql = sql & "Group by dtAll.Tgl, dtAll.Factory_Code, dtAll.CompleteCls, " & _
                    "dtAll.MaxResultDt, dtAll.MaxDailyDt " & strLine
    ...
    Else
        With frmEffGraph30
            ...
            Do While Not rsGraph.EOF
                ...
                .lblBarVal1(i) = Format(rsGraph!qtyDP, gs_formatQty)
                .lblBarVal2(i) = Format(rsGraph!qtyR, gs_formatQty)
                
                If rsGraph!qtyDP > .MaxQty Then .MaxQty = rsGraph!qtyDP
                If rsGraph!qtyR > .MaxQty Then .MaxQty = rsGraph!qtyR
                
                If IsNull(rsGraph!MaxDailyDt) Then .MaxDailyDt = 0 Else .MaxDailyDt = Day(rsGraph!MaxDailyDt)
                If IsNull(rsGraph!MaxResultDt) Then .MaxResultDt = 0 Else .MaxResultDt = Day(rsGraph!MaxResultDt)
                i = i + 1
                rsGraph.MoveNext
            Loop
