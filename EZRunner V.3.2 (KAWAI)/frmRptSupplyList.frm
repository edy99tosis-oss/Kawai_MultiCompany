...
Begin VB.Form frmRptSupplyList 
   BackColor       =   &H00FDDFE3&
   Caption         =   "Parts (Material) Supply List Report"
   ClientHeight    =   5640
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   10080
   ...
   EndProperty
   Icon            =   "frmRptSupplyList.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   5640
   ScaleWidth      =   10080
   ...
   End
End
Attribute VB_Name = "frmRptSupplyList"
    ...
    Else
        ...
        Else
          ...
          sql = "select im.finishgoodpart_cls, " & sqlfg1 & _
               ...
               "ps.childsupply_date, ps.BC_type, ps.BC40_No, ps.BC40_Date, rtrim(ISNULL(ParentItem_Code,''))ParentItem_Code, rtrim(ps.childitem_code) childitem_code, rtrim(im.MakerItem_Code) part_number, " & _
               "rtrim(im.item_name) childitem_name, RTRIM(ISNULL(MM.Description, ''))Model_Cls, " & _
               "rtrim(ps.lot_no) lot_no, isnull(ps.childrequirement_qty,0) childqty, ps.childunit_cls, rtrim(uc.description) description, " & _
               "RTRIM(COALESCE(ps.supplyrec_no,ps.sjno)) supplyrec_no, isnull(ps.do_no,'') do_no, rtrim(isnull(ps.remarks,'')) remarks " & _
               "From part_supply ps " & _
               "inner join item_master im on im.item_code=ps.childitem_code " & _
               "left join (SELECT Item_Code, Model_Cls FROM Item_Master) MC ON PS.ParentItem_Code = MC.Item_Code " & _
               "left join Model_Cls MM on MC.Model_Cls = MM.Model_Cls " & _
